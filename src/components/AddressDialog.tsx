...
export function AddressDialog({ isOpen, onClose, editingAddress }: AddressDialogProps) {
  ...
  // Check serviceability when coordinates change
  useEffect(() => {
  ...
  }, [formData.latitude, formData.longitude, formData.address_line_1, setManualLocation]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.latitude && formData.longitude && inDeliveryArea === false) {
      toast({

...
DROP FUNCTION IF EXISTS public.get_order_by_token(uuid, text);
DROP FUNCTION IF EXISTS public.get_lab_booking_by_token(uuid, text);
...
-- Create new tracking functions with token expiry validation and PII masking
...
BEGIN
  ...
  WHERE o.id = order_id 
    ...
    AND (o.tracking_token_expires_at IS NULL OR o.tracking_token_expires_at > now());
END;
$$;
...
BEGIN
  ...
  WHERE lb.id = booking_id 
    ...
    AND (lb.tracking_token_expires_at IS NULL OR lb.tracking_token_expires_at > now());
END;
$$;
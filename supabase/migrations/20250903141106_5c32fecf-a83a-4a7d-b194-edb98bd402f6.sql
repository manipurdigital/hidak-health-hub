...
create policy if not exists "Public can read lab reports"
on storage.objects
for select
using (bucket_id = 'lab-reports');

-- Allow authenticated users to upload files into a folder named with their user ID
create policy if not exists "Users can upload own lab reports"
on storage.objects
for insert
with check (
...
create policy if not exists "Users can update own lab reports"
on storage.objects
for update
using (
  bucket_id = 'lab-reports'
...
create policy if not exists "Users can delete own lab reports"
on storage.objects
for delete
using (
  bucket_id = 'lab-reports'
...
create policy if not exists "Users can insert their lab reports"
on public.lab_reports
for insert
with check (auth.uid() = user_id);

create policy if not exists "Users can update their lab reports"
on public.lab_reports
for update
using (auth.uid() = user_id);

create policy if not exists "Users can delete their lab reports"
on public.lab_reports
for delete
using (auth.uid() = user_id);

create policy if not exists "Users view their uploaded lab reports"
on public.lab_reports
for select
using (auth.uid() = user_id);

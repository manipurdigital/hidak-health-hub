...
-- Add performance index for geofence queries
CREATE INDEX IF NOT EXISTS idx_geofences_active_service_type 
ON geofences (is_active, service_type) 
WHERE is_active = true;

-- Add performance index for service_areas spatial queries
CREATE INDEX IF NOT EXISTS idx_service_areas_geom_gist 
ON service_areas USING GIST (geom) 
WHERE active = true;

-- Add index for lab_bookings center_id and date queries
CREATE INDEX IF NOT EXISTS idx_lab_bookings_center_date 
ON lab_bookings (center_id, booking_date) 
WHERE status != 'cancelled';

-- Add index for orders delivery center and date queries  
CREATE INDEX IF NOT EXISTS idx_orders_delivery_center_date 
ON orders (delivery_center_id, (created_at::date)) 
WHERE status != 'cancelled';

-- Enable RLS on any tables that don't have it (fix critical security issue)
...
CREATE POLICY IF NOT EXISTS "Restrict geography_columns access" ON geography_columns
FOR ALL USING (false);

CREATE POLICY IF NOT EXISTS "Restrict geometry_columns access" ON geometry_columns  
FOR ALL USING (false);

CREATE POLICY IF NOT EXISTS "Restrict spatial_ref_sys access" ON spatial_ref_sys
FOR ALL USING (false);
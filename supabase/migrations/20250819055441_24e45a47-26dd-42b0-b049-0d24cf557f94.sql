...
DROP FUNCTION IF EXISTS universal_search_v2(TEXT, INTEGER);
...
-- Create improved universal search function that prioritizes brand/trade name
...
BEGIN
  ...
  WITH medicine_search AS (
    ...
    ORDER BY rank_score DESC, m.name
    LIMIT max_per_group
  ),
  doctor_search AS (
    SELECT 

-- Fix infinite recursion in center_staff RLS policy and add core optimizations
...
-- Add performance indexes for key queries
...
CREATE INDEX IF NOT EXISTS idx_orders_delivery_center_created
ON orders (delivery_center_id, created_at) 
WHERE status != 'cancelled';